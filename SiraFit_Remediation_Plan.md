...
| Task                                                          | Action                                                                                                                                                                                                                          |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Stale 2FA TODO (`AuthContext.tsx:49`)                       | Resolved (94fc2cf): comment now references the remediation plan backlog instead of a floating TODO.                                                                                                                                                     |
| `frontend/src/lib/api/batch.test.ts`                        | Resolved (94fc2cf): removed the dead file — no Jest/Vitest runner exists in package.json.                                                    |
| `_app.match.tsx` / `_app.ranking.tsx`                     | Resolved (2026-08-22): product decision = keep separate (distinct workflows). No code change.                                                                                                 |
| `_app.analytics.market.tsx` / `_app.analytics.skills.tsx` | Resolved: premise was stale — both pages already fetch getAnalyticsMetrics with real computed data (backend generate_analytics_metrics). No action needed.                                                                                                                         |
| `backend/app/worker/`                                       | Verified (2026-08-22): celery_worker + celery_beat already in docker-compose.yml and docker-compose.test.yml; Dockerfile.celery present. |
| Error boundaries / Suspense                                   | Resolved (94fc2cf): added reusable RouteErrorBoundary, attached to _app.tsx and _app.jobs.tsx.                                                                                |
| Redis key growth                                              | Resolved (94fc2cf): redis service runs --maxmemory-policy allkeys-lru.                                                                                                                                 |

...
class AIJobAnalysisOutput(BaseModel):
    ...
    summary: str = Field(..., min_length=10, description="2-3 sentence summary of role and candidate fit")
    pros: List[str] = Field(default_factory=list, description="Strengths and strong alignment points")
    cons: List[str] = Field(default_factory=list, description="Weaknesses or potential mismatch points")
    skills_gap: List[str] = Field(default_factory=list, description="Missing required or preferred skills")
    key_requirements: List[str] = Field(default_factory=list, description="Top requirements of the position")
    seniority: str = Field(default="Mid", description="Seniority level: Junior, Mid, Senior, Staff, Lead, Director")
...
class AIResumeExperienceItem(BaseModel):
    ...
    bullets: List[str] = Field(default_factory=list, description="Bullet points detailing achievements and metrics")
...
class AIResumeOutput(BaseModel):
    ...
    summary: str = Field(..., min_length=20, max_length=2000, description="ATS-optimized professional summary")
    experience: List[AIResumeExperienceItem] = Field(default_factory=list, description="Tailored experience items")
    projects: List[AIResumeProjectItem] = Field(default_factory=list, description="Highlighted projects")
    skills: List[str] = Field(default_factory=list, description="Relevant skills for target job")
    education: List[AIResumeEducationItem] = Field(default_factory=list, description="Education records")
...
class AICoverLetterOutput(BaseModel):
    ...
    salutation: Optional[str] = Field(default="Dear Hiring Manager,", description="Professional salutation")
    body: str = Field(..., min_length=50, description="The complete cover letter body (paragraphs)")
    sign_off: Optional[str] = Field(default="Sincerely,", description="Closing sign-off, e.g. Sincerely,")
    key_points: List[str] = Field(default_factory=list, description="Key highlighted selling points used in the letter")

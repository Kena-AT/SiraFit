...
def generate_learning_plan(db: Session, profile: Profile, job: Job) -> Dict[str, Any]:
    ...
    profile_skills = {skill.name.lower() for skill in profile.skills} if profile.skills else set()
    ...
    stmt = select(SkillTaxonomy).where(
        ...
        SkillTaxonomy.canonical_key.in_([s.lower().replace(' ', '-') for s in missing_skill_names])
    )

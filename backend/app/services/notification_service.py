...
def send_notification_email(notification_id: uuid.UUID) -> bool:
    ...
    try:
        ...
        if not notification:
            logger.warning("notification_not_found", extra={"notification_id": str(notification_id)})
            return False
        
        user = db.query(User).filter(User.id == notification.user_id).first()
        if not user:
            logger.warning("user_not_found", extra={"user_id": str(notification.user_id)})
            return False
        ...
        try:
            ...
            logger.info("notification_email_sent", extra={"notification_id": str(notification_id)})
            return True
        except Exception as e:
            logger.exception("notification_email_failed", extra={"notification_id": str(notification_id), "error": str(e)})
            return False
    finally:
        db.close()
...
def check_and_send_reminders() -> int:
    ...
    try:
        ...
        applications = db.query(JobApplication).filter(
            ...
            JobApplication.status.notin_(["offer", "rejected", "withdrawn", "archived"])
        ).all()

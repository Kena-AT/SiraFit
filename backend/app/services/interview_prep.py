...
class InterviewQuestion(BaseModel):
    ...
    focus_skill: str = Field(..., description="The skill this question aims to evaluate, typically one the candidate is missing.")
    ideal_answer_points: List[str] = Field(..., description="Bullet points of what a good answer should include.")

class InterviewQuestions(BaseModel):
    questions: List[InterviewQuestion] = Field(..., description="List of 3 specific interview questions.", min_length=3, max_length=3)
...
async def generate_prep_questions(db: Session, user_id: uuid.UUID, profile: Profile, job: Job) -> Dict[str, Any]:
    """
    ...
    usage_count = db.query(func.count(AICompletion.id)).filter(
    ...
    if usage_count >= RATE_LIMIT_24H:
        raise RateLimitExceeded(f"Rate limit exceeded: You can only generate {RATE_LIMIT_24H} interview preps per 24 hours.")
    ...
    profile_skills_text = ", ".join([s.name for s in profile.skills]) if profile.skills else "None"
    ...
    if not candidates:
        raise RuntimeError("No AI candidates configured. Please configure an AI provider in Settings.")

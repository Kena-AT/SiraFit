...
export function validateProfile(profile: Profile): Record<string, string> {
  ...
  profile.certifications.forEach((cert: Certification, index: number) => {
    ...
    if (cert.credential_id && cert.credential_id.length > 255) {
      errors[`certifications[${index}].credential_id`] = 'Credential ID cannot exceed 255 characters';
    }

    if (cert.credential_url && cert.credential_url.length > 255) {
      errors[`certifications[${index}].credential_url`] = 'Credential URL cannot exceed 255 characters';
    }

    // Date validation
    if (cert.issue_date && cert.expiration_date && new Date(cert.issue_date) > new Date(cert.expiration_date)) {
      errors[`certifications[${index}].expiration_date`] = 'Expiration date cannot be before issue date';
    }
  });

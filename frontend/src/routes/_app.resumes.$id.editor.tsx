...
function ResumeEditorPage() {
  ...
  const handleArrayChange = <T extends Experience | Education | Skill | Project | Certification>(
    field: keyof Profile, index: number, subField: keyof T, value: any
  ) => {
    if (!profile) return;
  ...
  const handleAddItem = <T extends Experience | Education | Skill | Project | Certification>(
    field: keyof Profile, template: T
  ) => {
    if (!profile) return;
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className={`mb-4 ${saveStatus === 'saved' ? 'text-green-600' : saveStatus === 'error' ? 'text-red-600' : ''}`}>
        Status: {saveStatus}
      </div>
      {Object.keys(errors).length > 0 && <ValidationDisplay errors={errors} />}

      <SectionCard title="Personal Information" defaultOpen={true}>
        <Input label="First Name" value={profile.first_name || ''} onChange={(v) => handleFieldChange('first_name', v)} />
        <Input label="Last Name" value={profile.last_name || ''} onChange={(v) => handleFieldChange('last_name', v)} />
        <Input label="Headline" value={profile.headline || ''} onChange={(v) => handleFieldChange('headline', v)} />
        <Textarea label="Summary" value={profile.summary || ''} onChange={(v) => handleFieldChange('summary', v)} />
      </SectionCard>

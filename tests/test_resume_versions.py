...
def test_api_versions_list_and_diff(
    ...
    # 1. Base version exists
    base_v = create_base_version_from_profile(db, test_resume, test_profile)

    # 2. Add tailored version
    tailored_v = ResumeVersion(
        resume_id=test_resume.id,
        version_number=2,
        content=json.dumps(
    ...
    # User A lists versions -> 200
    res = client.get(f"/api/v1/resumes/{test_resume.id}/versions", headers=headers_user)
    ...
    diff_res = client.get(
        f"/api/v1/resumes/{test_resume.id}/versions/{base_v.id}/diff/{tailored_v.id}",
        headers=headers_user,
    )
    assert diff_res.status_code == 200
    diff_data = diff_res.json()
    assert diff_data["has_changes"] is True
    assert "Go" in diff_data["sections"]["skills"]["added"]

    # User B attempts to diff User A's versions -> 404
    diff_b = client.get(
        f"/api/v1/resumes/{test_resume.id}/versions/{base_v.id}/diff/{tailored_v.id}",
    ...
    # User A reverts to base_v -> 201
    rev_res = client.post(
        f"/api/v1/resumes/{test_resume.id}/versions/{base_v.id}/revert",
        headers=headers_user,
    )
    assert rev_res.status_code == 201
    rev_data = rev_res.json()
    assert rev_data["source_type"] == "revert"
    assert rev_data["version_number"] == 3
    assert rev_data["parent_version_id"] == str(base_v.id)

    # User B attempts to revert User A's version -> 404
    rev_b = client.post(
        f"/api/v1/resumes/{test_resume.id}/versions/{base_v.id}/revert",
        headers=headers_other,
    )
...
def test_api_application_cross_user_rejection(
    ...
    # other_user tries to create application using test_user's resume version -> 400
    app_res = client.post(
        "/api/v1/applications/",
        headers=headers_other,
        json={
            "job_id": str(test_job.id),
            "resume_version_id": str(base_v.id),
        },
    )
    assert app_res.status_code == 400
    assert "not belong to you" in app_res.json()["detail"]
